import sql from "../config/db.js"; // PostgreSQL sql
import cloudinary from "../config/cloudinary.js";
import getDataUri from "../utils/getDataUri.js";

export const applyLoan = async (req, res) => {
  try {
    const { user_id, loan_type, loan_amount, tenure } = req.body;

    if (!user_id || !loan_type || !loan_amount || !tenure) {
      return res.status(400).json({ message: "All fields are required" });
    }

    const allowedLoanTypes = ["home", "education", "personal", "vehicle", "gold", "property", "agriculture", "business"];
    ...
    const query = `
      INSERT INTO loan_req (user_id, loan_type, loan_amount, tenure, documents)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *;
    `;
    const values = [user_id, loan_type, loan_amount, tenure, uploadedFiles];
