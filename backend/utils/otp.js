...
import { formatNumber } from "./whatsapp.js";
...
export const sendWhatsappOTP = async (mobile, otp) => {
  try {
    if (!global.client) throw new Error("WhatsApp client not initialized.");

    const finalNumber = formatNumber(mobile);
    const msg = `Your verification OTP is: *${otp}*`;

    await global.client.sendMessage(finalNumber, { text: msg });
